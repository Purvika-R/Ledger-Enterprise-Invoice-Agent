...
export default function AnalyticsDashboard({ onNavigate }: Props) {
  ...
  const stats = [["Total Invoices", data?.total_invoices ?? 0], ["Known Vendors", data?.known_vendors ?? 0], ["Average Confidence", `${data?.average_confidence ?? 0}%`], ["Validation Success", `${data?.validation_success_rate ?? 0}%`]];
